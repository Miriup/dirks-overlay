...
src_install() {
	...
	sed -i -e "s:${ED}::g" \
		"${ED}/usr/bin/botan-config" \
		"${ED}/usr/$(get_libdir)/pkgconfig/botan-1.8.pc" || die "sed failed"
	mv "${ED}/usr/share/doc/Botan-${PV}" "${ED}/usr/share/doc/${PF}" || die "could not rename directory"
}
