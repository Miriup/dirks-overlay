...
src_install() {
	...
	if ! use snmp; then
		rm "${ED}"/usr/$(get_libdir)/nagios/plugins/check_if{operstatus,status} \
			|| die "Failed to remove SNMP check plugins"
	fi

	mv "${S}"/contrib "${ED}"/usr/$(get_libdir)/nagios/plugins/contrib

	if ! use jabber; then
		rm "${ED}"usr/$(get_libdir)/nagios/plugins/contrib/nagios_sendim.pl \
	...
	chmod -R o-rwx "${ED}"/usr/$(get_libdir)/nagios/plugins \
		|| die "Failed chmod of ${ED}usr/$(get_libdir)/nagios/plugins"

	if use suid ; then

		chmod 04710 "${ED}"/usr/$(get_libdir)/nagios/plugins/{check_icmp,check_ide_smart,check_dhcp} \
