...
# $Header: /var/cvsroot/gentoo-x86/app-emulation/wine/wine-0.9.48.ebuild,v 1.1 2007/10/27 19:56:21 vapier Exp $
...
IUSE="alsa aqua cups dbus esd hal jack jpeg lcms ldap nas ncurses opengl oss scanner xml X"
RESTRICT="test" #72375

ECVS_AUTH="pserver"
ECVS_SERVER="darwine.cvs.sourceforge.net:/cvsroot/darwine"
ECVS_USER="anonymous"
ECVS_PASS=""
ECVS_MODULE="drivers"
...
src_unpack() {
	...
	# Quartz patch as per http://wiki.winehq.org/MacOSX/QuartzDriver
	if use aqua; then
		cvs_src_unpack
		cd "${S}"
		cp -r ${WORKDIR}/drivers/dlls/quartzdrv ${S}/dlls/winequartz.drv
		epatch ${WORKDIR}/drivers/add_winequartz.drv_to_the_build_system.diff
		eautoconf
	fi
}
...
src_compile() {
	...
	# The Carbon headers on Darwin expect us to have SSE FP math enabled.
	if [[ "${MY_PN}" == "gvim" ]] && [[ ${CHOST} == i686-*-darwin* ]]; then
		append-flags -msse3
	fi
	#
	# Some final extra tweaks needed for Carbon on Darwin that can't hurt
	# somewhere else. The preprocessor doesn't compile, so no need to
	# specify explicitely which SSE instruction set to use.
	#
	if [[ ${CHOST} == i686-*-darwin* ]]; then
