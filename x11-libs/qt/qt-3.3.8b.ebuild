...
# 
SRC_URI="!aqua? ( ftp://ftp.trolltech.com/qt/source/qt-x11-${SRCTYPE}-${PV}.tar.gz )
	aqua? ( ftp://ftp.trolltech.com/qt/source/qt-mac-${SRCTYPE}-${PV}.tar.gz )
	immqt? ( mirror://gentoo/${IMMQT_P}.diff.bz2 )
	immqt-bc? ( mirror://gentoo/${IMMQT_P}.diff.bz2 )"
LICENSE="|| ( QPL-1.0 GPL-2 )"

SLOT="3"
KEYWORDS="~amd64-linux ~ia64-linux ~mips-linux ~x86-linux ~x86-fbsd ~x86-macos"
IUSE="cups debug doc examples firebird gif ipv6 mysql nas nis odbc opengl postgres sqlite xinerama immqt immqt-bc aqua"

DEPEND="!aqua? ( x11-libs/libXcursor
	x11-libs/libXi
	x11-libs/libXrandr
	x11-libs/libSM
...
src_unpack() {
	...
	epatch ${FILESDIR}/${P%b}-uic-fix.patch

	# Patch for mysql unload crash (Bug #171883)
	#use aqua || epatch ${FILESDIR}/${P}-mysql-unload-crash-x11.diff
	epatch ${FILESDIR}/${P}-mysql-unload-crash.diff

	# KDE related patches
	epatch ${FILESDIR}/0001-dnd_optimization.patch
	use aqua || ${FILESDIR}/0001-dnd_optimization-x11.patch
	use aqua || epatch ${FILESDIR}/0002-dnd_active_window_fix.patch
	...
	use aqua || epatch ${FILESDIR}/0048-qclipboard_hack_80072.patch

	# possible rce, CVE-2007-3388
	# Dirk: seems to be applied upstream
	#epatch ${FILESDIR}/0081-format-string-fixes.diff

	# Bug #192472
	# Dirk: seems to be applied upstream
	#epatch ${FILESDIR}/${P%b}-unicode-off-by-one.patch

	# ulibc patch (bug #100246)
	epatch ${FILESDIR}/qt-ulibc.patch

	# xinerama patch: http://ktown.kde.org/~seli/xinerama/
	use aqua || epatch "${FILESDIR}/${P}-seli-xinerama.patch"

	# Dirk: applied upstream
	...
	# [[ $(gcc-major-version)$(gcc-minor-version) -ge 41 ]] && \
		epatch "${FILESDIR}/${P%b}-visibility.patch"
		use aqua || epatch "${FILESDIR}/${P%b}-visibility-x11.patch"
	...
	# Dirk: Hmmm, have we been a little bit too optimistic below?
	#PROFILE=`echo $(tc-getCXX) | sed "s|^.*-\([a-z]\{1,\}\)[0-9]*-\([^-]\{1,\}\)|\1-\2|"`
	#einfo Patching your favorite compiler flags into mkspecs/${PROFILE}/qmake.conf
	#sed -i -e "s|\(QMAKE_CFLAGS_RELEASE[ 	]*= \)|${CFLAGS}|" mkspecs/${PROFILE}/qmake.conf
	sed -i -e "s|\(QMAKE_CFLAGS[ 	]*= \)|\1-msse3 |" mkspecs/darwin-g++/qmake.conf
}

src_compile() {
	export SYSCONF=${ED}${QTBASE}/etc/settings
	...
	! use aqua && use xinerama    && myconf="${myconf} -xinerama" || myconf="${myconf} -no-xinerama"
	...
	use aqua || myconf="${myconf} -xrender -xft "
	...
	./configure -sm -thread -stl -system-libjpeg -verbose -largefile \
		...
		-system-libpng -platform ${PLATFORM} -xplatform \
		${PLATFORM} -prefix ${QTBASE} -libdir ${QTBASE}/$(get_libdir) \
		-fast -no-sql-odbc ${myconf} -dlopen-opengl || die

	# No idea how to get it in otherwise
	einfo Pushing our CFLAGS into QT3
	sed -i -e "s|^\(QMAKE_CFLAGS[ 	]*= \)|\1$CFLAGS|" qmake/Makefile
...
src_install() {
	...
	if use ppc-macos; then
		...
		cp -fR lib/*.{dylib,la,a} ${ED}/${QTBASE}/$(get_libdir) || die

		cd ${ED}/${QTBASE}/$(get_libdir)
	...
	cp include/* ${ED}/${QTBASE}/include/
	cp include/private/* ${ED}/${QTBASE}/include/private/
...
EOF
	...
	if use examples; then
		...
		cp -r ${S}/examples ${ED}${QTBASE}/
		cp -r ${S}/tutorial ${ED}${QTBASE}/
	fi
	...
	sed -e "s:${S}:${QTBASE}:g" \
		${S}/.qmake.cache > ${ED}${QTBASE}/.qmake.cache
